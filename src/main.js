...
module.exports = async (newDirPath, serverUrl, currentVersion)=>{
    ...
    var configResults = await firebird.query(`SELECT chave,valor FROM REPLIC_CONFIG `, [], ["chave","valor"]);
    var config = {};
    ...
    async function run(){
        ...
        try{
            ...
            var canPush = false;
            var configAuth = fs.readFileSync(dirPath+"/sincronizador.cfg", {encoding:'utf8'});
            if(configAuth == ""){
            ...
            await firebird.execute("select rdb$set_context('USER_SESSION', 'DONT_TRIGGER_REPLIC', 'S') from rdb$database", []);

            var result = await axios.post(serverUrl+"/modifications",{
                action: "get",
                since: isNaN(config.date_since_last_pull) ? '0' : config.date_since_last_pull,
            ...
            if(result.data.result){
                if(result.data.data.length > 0){
                    for(var i=0;i<result.data.data.length;i++){
                        ...
                        if(modification.situacao == 0 || modification.situacao == 1){ // inserido ou atualizado
                            ...
                            for(var colkey in modification.dados){
                                if(colkey.toUpperCase() == "SINC_UUID" && modification.dados[colkey] !== null){
                                    hasUUIDOnDados = true;
                                }
                                if(typeof modification.dados[colkey] !== "object" || (modification.forceDados && colkey in modification.forceDados)){
                            ...
                            /*console.log(`
                                ...
                                VALUES
                                (${modificationValues.join("','")}) MATCHING (SINC_UUID)
                            `);*/
                            await firebird.execute(`
                                UPDATE OR INSERT INTO ${modification.tabela}
                                (${modification.colunas.join(",")})
                                VALUES
                                (${tabelaFieldsMarkers.join(",")}) MATCHING (SINC_UUID)
                            `, modificationValues);
                            
                            if (modification.tabela == 'MOBILE_PEDIDO' || modification.tabela == 'MOBILE_PEDIDO_PRODUTOS') {
                                idPedidos =  Object.assign(idPedidos, await trataPedidos.trata(firebird, modification, idPedidos));
                            } else {
                                await firebird.execute("UPDATE OR INSERT INTO replic_data_status (UUID,Tabela,data_operacao,situacao,sincronizado) VALUES (?,?,?,?,1) MATCHING(UUID)", [modification.uuid, modification.tabela, new Date(modification.data_operacao), modification.situacao]);
                            }
    
                        }else if(modification.situacao == 2){ //deletado
            ...
            await firebird.execute("select rdb$set_context('USER_SESSION', 'DONT_TRIGGER_REPLIC', null) from rdb$database", []);
            ...
            if(canPush){
                ...
                select first 20 uuid, tabela, data_operacao, situacao, sincronizado
                ...
                for(var key in results){
                        ...
                        var resultObj = await firebird.query(`
                            SELECT first 1 ${colunas.join(",")} FROM ${toSend.tabela} WHERE SINC_UUID = ?
                        `, [toSend.uuid], colunas);
                ...
                if(modificationsToSendToCloud.length > 0){
                    ...
                    if(result.data.result == true){
                        ...
                        await firebird.execute(`UPDATE replic_data_status SET sincronizado = 1 WHERE uuid IN (${modificationsMarkers.join(",")})`, modificationsUUIDs);
                    }
                }
            }
